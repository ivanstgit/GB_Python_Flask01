...
class UserPermission(PermissionMixin):
    ...
    def patch_data(self, *args, data: dict = None, obj: User = None, user_permission: PermissionUser = None, **kwargs) -> dict:  # type: ignore
        permission_for_patch = user_permission.permission_for_patch_permission(model=User)

        if obj.id == current_user.id or current_user.is_staff:
            return {
                i_key: i_val
